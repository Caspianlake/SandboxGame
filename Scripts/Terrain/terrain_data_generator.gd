...
func generate_chunk(chunk_key: Vector3i) -> void:
	...
	for bx in range(0, chunk_size.x + 1):
		for bz in range(0, chunk_size.z + 1):
			var cell_height: int = floori(remap(main_noise.get_noise_2d(bx+block_offset.x,bz+block_offset.z),-1,1,0,chunk_size.y-1))
